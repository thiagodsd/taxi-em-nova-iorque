...
def novas_features_simples(data: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # features temporais
    data["pickup_dt"] = pd.to_datetime(
        data["pickup_datetime"], format="%Y-%m-%d %H:%M:%S", errors="ignore"
    )
    ...
    data["dist_manhattan_meter"] = data.apply(
        lambda x: lat_lon_converter(
...
def novas_features_complexas(data: pd.DataFrame, fit_obs: bool, frac_obs: float) -> Dict:
    ...
    if fit_obs == True:
        ...
        return dict(features_complexas=data, modelo_dbscan=db, modelo_kmeans=kmeans)
    else:
        return data
